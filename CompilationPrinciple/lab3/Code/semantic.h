...
struct Var {
    ...
    VarP next;
    int variable_cnt;
    int from_func;  //用于lab3的variable以及address类型的区分
    // 0为是，1为否
};
struct Function {
    char* name;
...
// create a table
VarP create_var(int in_struct, FieldListP field, VarP next, int line, int is_from_fun);
// create a function
...
void insert_var_table(FieldListP field_list, int line, int in_struct, int is_from_fun);
